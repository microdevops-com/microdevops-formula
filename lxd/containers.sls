...
lxd_container_device_set_{{ a_loop.index }}_{{ b_loop.index }}_{{ loop.index }}:
  cmd.run:
    - name: '/snap/bin/lxc config device set {{ container_name }} {{ device_name }} {{ device_param_key }} {{ device_param_val }}'
...
lxd_config_set_{{ a_loop.index}}_{{ loop.index }}:
  cmd.run:
    - name: '/snap/bin/lxc config set {{ container_name }} {{ config_key }} {{ config_val }}'
...
lxd_profile_assign_{{ loop.index }}:
  cmd.run:
    - name: '/snap/bin/lxc profile assign {{ container_name }} {% for profile in container_val['profiles'] %}{{ profile }}{{ "," if not loop.last else ""}}{% endfor %}'

      {%- if 'allow_stop_start' in pillar['lxd'] and pillar['lxd']['allow_stop_start'] %}
lxd_start_{{ loop.index }}:
  cmd.run:
    - name: '/snap/bin/lxc start {{ container_name }}'
      {%- endif %}
...
lxd_copy_bootstrap_script_{{ a_loop.index}}_{{ b_loop.index}}_{{ loop.index }}:
  cmd.run:
    - name: '/snap/bin/lxc file push /tmp/lxd_bootstrap_{{ bootstrap_script }} {{ container_name }}/etc/bootstrap_{{ bootstrap_script }}'
