  ...
  cmd.run:
    - name: |
        ...
        USER_ID=$(curl -sS -u "{{ rancher_val["bearer_token"] }}" \
          ...
          "https://{{ rancher_val["cluster_domain"] }}/v3/users" | jq .data | jq -r .[].id | head -n 1)
        if [[ -n ${USER_ID} ]]; then
          # Update user
          curl -sS -u "{{ rancher_val["bearer_token"] }}" -X PUT -H 'Accept: application/json' -H 'Content-Type: application/json' -d '{
          ...
          }' "https://{{ rancher_val["cluster_domain"] }}/v3/users/${USER_ID}"
        else
          # Create user
          curl -sS -u "{{ rancher_val["bearer_token"] }}" -X POST -H 'Accept: application/json' -H 'Content-Type: application/json' -d '{
            "username": "{{ user["username"] }}",
            "description": "{{ user["description"] }}",
            "me": false,
            "mustChangePassword": {{ 'true' if user["must_change_password"] else 'false' }},
            ...
            "password": "{{ user["password"] }}",
            "principalIds": []
          }' "https://{{ rancher_val["cluster_domain"] }}/v3/users"
        fi
        ...
        BINDINGS=$(curl -sS -u "{{ rancher_val["bearer_token"] }}" \
          --get --data-urlencode "userId=${USER_ID}" \
          "https://{{ rancher_val["cluster_domain"] }}/v3/globalrolebindings" | jq .data | jq -r .[].id)
        for BINDING in ${BINDINGS}; do
          curl -sS -u "{{ rancher_val["bearer_token"] }}" -X DELETE -H 'Accept: application/json' "https://{{ rancher_val["cluster_domain"] }}/v3/globalrolebindings/${BINDING}"
        done
        # Create membership
        curl -sS -u "{{ rancher_val["bearer_token"] }}" -X POST -H 'Accept: application/json' -H 'Content-Type: application/json' -d '{
          "globalRoleId": "{{ user["global_permissions"] }}",
          "userId": "'${USER_ID}'"
        }' "https://{{ rancher_val["cluster_domain"] }}/v3/globalrolebindings"
...
rancher_user_token_dir_{{ a_loop.index }}_{{ loop.index }}:
  file.directory:
    - name: /opt/rancher/clusters/{{ rancher_val["cluster_name"] }}/tokens/{{ user["username"] }}
    - mode: 700
    - makedirs: True
  ...
  cmd.run:
    - name: |
        ...
        USER_BEARER_TOKEN=$(curl -sS -X POST -H 'Accept: application/json' -H 'Content-Type: application/json' -d '{
            ...
            "ttl": 60000
          }' "https://{{ rancher_val["cluster_domain"] }}/v3-public/localProviders/local?action=login" | jq -r .token)
        # Check if token with the needed description exist, make new token if not
        TOKEN=$(curl -sS -u "${USER_BEARER_TOKEN}" --get "https://{{ rancher_val["cluster_domain"] }}/v3/tokens" \
          ...
          }' "https://{{ rancher_val["cluster_domain"] }}/v3/tokens" | jq -r .token)
        # Save token to file
        if [[ -z ${TOKEN} ]]; then
          echo token not created and not saved
        else
          echo ${TOKEN} > /opt/rancher/clusters/{{ rancher_val["cluster_name"] }}/tokens/{{ user["username"] }}/{{ token["description"] }}
          echo token saved to /opt/rancher/clusters/{{ rancher_val["cluster_name"] }}/tokens/{{ user["username"] }}/{{ token["description"] }}
        fi
              {%- endfor %}
            {%- endif %}
          {%- endfor %}
  ...
  cmd.run:
    - name: |
        ...
        PROJECT_ID=$(curl -sS -u "{{ rancher_val["bearer_token"] }}" \
          ...
          "https://{{ rancher_val["cluster_domain"] }}/v3/projects" | jq .data | jq -r .[].id | head -n 1)
        if [[ -n ${PROJECT_ID} ]]; then
          ...
          curl -sS -u "{{ rancher_val["bearer_token"] }}" -X PUT -H 'Accept: application/json' -H 'Content-Type: application/json' -d '{
            ...
            "baseType": "project",
            "clusterId": "local"
          }' "https://{{ rancher_val["cluster_domain"] }}/v3/projects/${PROJECT_ID}"
        else
          # Create project
          curl -sS -u "{{ rancher_val["bearer_token"] }}" -X POST -H 'Accept: application/json' -H 'Content-Type: application/json' -d '{
            "name": "{{ project["name"] }}",
            ...
            "baseType": "project",
            "clusterId": "local"
          }' "https://{{ rancher_val["cluster_domain"] }}/v3/projects"
        fi
...
rancher_project_member_user_{{ a_loop.index }}_{{ loop.index }}:
  cmd.run:
    - name: |
        ...
        USER_PRINCIPLE_ID=$(curl -sS -u "{{ rancher_val["bearer_token"] }}" --get \
          --data-urlencode "name={{ member["name"] }}" \
          "https://{{ rancher_val["cluster_domain"] }}/v3/users" | jq .data[] | jq -r .principalIds[] | grep googleoauth_user | head -n 1)
        ...
        BINDINGS=$(curl -sS -u "{{ rancher_val["bearer_token"] }}" \
          ...
          "https://{{ rancher_val["cluster_domain"] }}/v3/projectroletemplatebindings" | jq .data | jq -r .[].id)
        for BINDING in ${BINDINGS}; do
          curl -sS -u "{{ rancher_val["bearer_token"] }}" -X DELETE -H 'Accept: application/json' "https://{{ rancher_val["cluster_domain"] }}/v3/projectroletemplatebindings/${BINDING}"
        done
        # Create membership
        curl -sS -u "{{ rancher_val["bearer_token"] }}" -X POST -H 'Accept: application/json' -H 'Content-Type: application/json' -d '{
          ...
          "userPrincipalId": "'${USER_PRINCIPLE_ID}'",
                {%- endif %}
          "userId": "'${USER_ID}'"
        }' "https://{{ rancher_val["cluster_domain"] }}/v3/projectroletemplatebindings"
...
rancher_project_member_group_{{ a_loop.index }}_{{ loop.index }}:
  cmd.run:
    - name: |
        ...
        GROUP_ID=$(curl -sS -u "{{ rancher_val["bearer_token"] }}" \
          --get \
          "https://{{ rancher_val["cluster_domain"] }}/v3/principals" | jq '.data[] | select(.loginName == "{{ member["groupname"] }}")' | jq -r .id | head -n 1)
        if [[ -z ${GROUP_ID} ]]; then
