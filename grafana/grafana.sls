...
{% if pillar['grafana']['domains'] is defined %}
nginx_files_1:
  file.managed:
...
{% elif pillar['grafana']['default_domain'] is defined %}
...
nginx_files_1:
  ...
  {%- for domain in pillar['grafana']['default_domain'] %}
            server {
                listen 443 ssl;
                server_name {{ domain['name'] }};
                root /opt/grafana/{{ domain['name'] }};
...
nginx_domain_index_{{ loop.index }}:
  file.managed:
    ...
    - contents: |
    {%- for instance in domain['instances'] %}
        <meta http-equiv="refresh" content="1; url='https://{{ domain['name'] }}/{{ instance['name'] }}'" />
    {%- endfor %}
  {%- endfor %}
