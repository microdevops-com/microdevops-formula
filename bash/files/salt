...
_salt_get_grains(){
    if [ "$1" = 'local' ] ; then
        salt-call --log-level=error --out=txt -- grains.ls | sed -e "s/u'/'/g" | sed  's/^.*\[//' | tr -d ",']" |sed 's:\([a-z0-9]\) :\1\: :g'
    else
      salt '*' --timeout 2 --hide-timeout --log-level=error --out=txt -- grains.ls | sed -e "s/u'/'/g" | sed  's/^.*\[//' | tr -d ",']" |sed 's:\([a-z0-9]\) :\1\: :g'
    fi
}

_salt_get_grain_values(){
    if [ "$1" = 'local' ] ; then
        salt-call --log-level=error --out=txt -- grains.item $1 | sed -e "s/u'/'/g" |sed 's/^\S*:\s//' |grep -v '^\s*$'
    else
        salt '*' --timeout 2 --hide-timeout --log-level=error --out=txt -- grains.item $1 | sed -e "s/u'/'/g" |sed 's/^\S*:\s//' |grep -v '^\s*$'
    fi
}
...
_salt_list_functions(){
    ...
    if [ "$1" = 'local' ] ; then
        salt-call --log-level=quiet --out=txt -- sys.list_functions | sed -e "s/u'/'/g" \
          | sed "s/^.*\[//;s/[],']//g;s/ /\n/g" \
          | sort -u
    else
...
_salt_list_states(){
    if [ "$1" = 'local' ] ; then
        salt-call --log-level=quiet --out=txt -- state.show_state_usage \
          | sed -e "s/u'/\n'/g" \
          | grep -v -e "count_used" -e "'top'" -e "'used'" -e "count_unused" -e "count_all" -e "'unused'" -e "'base'" -e "local:" \
          | sed -e "s/^'//" -e "s/'.*$//" \
          | sort -u
    else
...
_salt_get_coms() {
	...
	_salt_list_states $1 >> "${_salt_cache_functions}"
    fi
