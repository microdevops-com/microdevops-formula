...
#php 8.3
    {%- if (pillar['php-fpm']['version_8_3'] is defined) and (pillar['php-fpm']['version_8_3'] is not none) and (pillar['php-fpm']['version_8_3']) %}
php-fpm_repo_deb_8_3:
  pkgrepo.managed:
    - name: deb http://ppa.launchpad.net/ondrej/php/ubuntu {{ grains['oscodename'] }} main
    - dist: {{ grains['oscodename'] }}
    - file: /etc/apt/sources.list.d/ondrej-ubuntu-php-{{ grains['oscodename'] }}.list
...
php-fpm_8_3_installed:
  pkg.installed:
    - pkgs:
      ...
      {%- if (pillar['php-fpm']['modules'] is defined) and (pillar['php-fpm']['modules'] is not none) %}
        {%- if (pillar['php-fpm']['modules']['php8_3'] is defined) and (pillar['php-fpm']['modules']['php8_3'] is not none) %}
php-fpm_8_3_modules_installed:
  pkg.installed:
    - pkgs:
          {%- for pkg_name in pillar['php-fpm']['modules']['php8_3'] %}
            {%- if (pkg_name != 'php8.3-ioncube') %} # TODO: now (2023-12-07 12:41) there is no ionCube for php8.3
      - {{ pkg_name }}
            {%- endif %}
          {%- endfor %}

          {%- for pkg_name in pillar['php-fpm']['modules']['php8_3'] %}
            {%- if (pkg_name == 'php8.3-ioncube') %}
php-fpm_8_3_modules_ioncube_1:
  file.managed:
    - name: '/usr/lib/php/8.3-ioncube/ioncube_loader_lin_8.3.so'
    - makedirs: True
    - user: root
    - group: root
    - source: 'https://microdevopsformula.s3.eu-central-1.amazonaws.com/php-fpm/ioncube/ioncube_loader_lin_8.3.so'
    - skip_verify: True

php-fpm_8_3_modules_ioncube_2:
  file.managed:
    - name: '/etc/php/8.3/mods-available/ioncube.ini'
    - user: root
    - group: root
    - contents: |
        zend_extension=/usr/lib/php/8.3-ioncube/ioncube_loader_lin_8.3.so

php-fpm_8_3_modules_ioncube_3:
  file.symlink:
    - name: '/etc/php/8.3/fpm/conf.d/00-ioncube.ini'
    - target: '/etc/php/8.3/mods-available/ioncube.ini'
            {%- endif %}
          {%- endfor %}
