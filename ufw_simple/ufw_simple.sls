  ...
  file.managed:
    ...
    - defaults:
      ...
      {%- if (pillar['ufw_simple']['custom'] is defined) and (pillar['ufw_simple']['custom'] is not none) %}
        ...
        custom_nat: {{ pillar['ufw_simple']['custom']['nat'] | yaml_encode }}
        {%- else %}
        custom_nat: '# empty'
        {%- endif %}
      {%- else %}
      ...
      {%- endif %}
      {%- if (pillar['ufw_simple']['custom'] is defined) and (pillar['ufw_simple']['custom'] is not none) %}
        {%- if (pillar['ufw_simple']['custom']['filter'] is defined) and (pillar['ufw_simple']['custom']['filter'] is not none) %}
        custom_filter: {{ pillar['ufw_simple']['custom']['filter'] | yaml_encode }}
        {%- else %}
        custom_filter: '# empty'
        {%- endif %}
      {%- else %}
...
ufw_simple_reset:
  cmd.run:
    - name: 'ufw reset --force && sleep 5 && ufw enable'
    - runas: root
    {%- endif %}
