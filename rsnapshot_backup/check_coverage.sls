...
{% if
     data_dirs_to_find[grains['oscodename']] is defined and data_dirs_to_find[grains['oscodename']] is not none and
     data_dirs_to_skip[grains['oscodename']] is defined and data_dirs_to_skip[grains['oscodename']] is not none and
     db_ps[grains['oscodename']] is defined and db_ps[grains['oscodename']] is not none
%}
              ...
              {%- for backup_data_item in backup_item['data'] %}
                ...
                {%- if backup_data_item in data_expand %}
                  {%- if dir in data_expand[backup_data_item] %}
                    ...
                    {%- if backup_item['local_backups_suppress_reason'] is defined and backup_item['local_backups_suppress_reason'] is not none %}
                      {%- do data_dirs_backup_found[dir].update({'local': True}) %}
                    {%- endif %}
                    {%- if backup_item['remote_backups_suppress_reason'] is defined and backup_item['remote_backups_suppress_reason'] is not none %}
      ...
      {%- if salt['cmd.retcode'](cmd, python_shell=True) == 0 %}
        ...
        {%- for backup_item in pillar['rsnapshot_backup']['sources'][grains['id']] %}
          ...
          {%- if backup_item['type'] == 'SUPPRESS_COVERAGE' %}
            {%- for backup_data_item in backup_item['data'] %}
              {%- if
                    backup_data_item == db and
                    backup_item['local_backups_suppress_reason'] is defined and backup_item['local_backups_suppress_reason'] is not none
              %}
                {%- do db_backup_found[db].update({'local': True}) %}
              {%- endif %}
              {%- if
                    backup_data_item == db and
                    backup_item['remote_backups_suppress_reason'] is defined and backup_item['remote_backups_suppress_reason'] is not none
              %}
