...
#   - On a fresh install the seaweedfs container is Running right after `docker-compose up -d`,
#     but the filer inside still needs a few seconds to bootstrap; the wait loop below covers that.
...
sentry_seaweedfs_ttl_{{ bucket }}:
  cmd.run:
    ...
    - name: |
        ...
        # 1. Wait for the filer: fs.configure answers {"version":N,"locations":[...]} once it is up.
        for i in $(seq 1 30); do
          docker-compose exec -T seaweedfs sh -c "echo 'fs.configure' | weed shell" 2>/dev/null | grep -q '"locations"' && break
          echo "seaweedfs filer not ready (attempt ${i}/30), sleeping 5s"; sleep 5
          [ "$i" -eq 30 ] && { echo "seaweedfs filer did not become ready" >&2; exit 1; }
        done
        ...
        # 3. Verify. After stripping whitespace our entry reads {"locationPrefix":"/buckets/<b>/",...,"ttl":"<days>d"}.
    ...
    # Don't attempt anything if the seaweedfs container isn't up at all.
