...
# docker run --rm --volumes-from sentry-self-hosted-kafka-1		-v /opt/sentry/backup/volumes/:/backup ubuntu tar cf /backup/sentry-self-hosted-kafka-1.tar		/var/lib/kafka/data /etc/kafka/secrets /var/lib/kafka/log
  ...
  docker run --rm --volumes-from sentry-self-hosted-redis-1		-v /opt/sentry/backup/volumes/:/backup ubuntu tar cf /backup/sentry-self-hosted-redis-1.tar		/data
  docker run --rm --volumes-from sentry-self-hosted-smtp-1		-v /opt/sentry/backup/volumes/:/backup ubuntu tar cf /backup/sentry-self-hosted-smtp-1.tar		/var/spool/exim4 /var/log/exim4
# docker run --rm --volumes-from sentry-self-hosted-symbolicator-1	-v /opt/sentry/backup/volumes/:/backup ubuntu tar cf /backup/sentry-self-hosted-symbolicator-1.tar	/data
  ...
  if docker ps -a --format '{{.Names}}' | grep -q 'sentry-self-hosted-seaweedfs-1'; then
    docker run --rm --volumes-from sentry-self-hosted-seaweedfs-1       -v /opt/sentry/backup/volumes/:/backup ubuntu tar cf /backup/sentry-self-hosted-seaweedfs-1.tar /data
  fi
  ##
