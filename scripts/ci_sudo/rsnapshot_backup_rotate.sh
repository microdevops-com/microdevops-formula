...
if [ "${RSNAPSHOT_BACKUP_TYPE}" = "SSH" ]; then
	# Monthly
	stdbuf -oL -eL echo ---
	stdbuf -oL -eL            echo CMD: ssh -o BatchMode=yes -o StrictHostKeyChecking=no -p ${TARGET_PORT} ${TARGET} "bash -c 'exec > >(tee /opt/sysadmws/rsnapshot_backup/rsnapshot_backup.log); exec 2>&1; if [ $(date +%d) = 01 ]; then /opt/sysadmws/rsnapshot_backup/rsnapshot_backup.sh monthly; fi'"
	( set -o pipefail && stdbuf -oL -eL ssh -o BatchMode=yes -o StrictHostKeyChecking=no -p ${TARGET_PORT} ${TARGET} "bash -c 'exec > >(tee /opt/sysadmws/rsnapshot_backup/rsnapshot_backup.log); exec 2>&1; if [ $(date +%d) = 01 ]; then /opt/sysadmws/rsnapshot_backup/rsnapshot_backup.sh monthly; fi'" | ccze -A | sed -e 's/33mNOTICE/32mNOTICE/' ) || GRAND_EXIT=1
	# Weekly
	stdbuf -oL -eL echo ---
	stdbuf -oL -eL            echo CMD: ssh -o BatchMode=yes -o StrictHostKeyChecking=no -p ${TARGET_PORT} ${TARGET} "bash -c 'exec > >(tee /opt/sysadmws/rsnapshot_backup/rsnapshot_backup.log); exec 2>&1; if [ $(date +%u) = 1 ]; then /opt/sysadmws/rsnapshot_backup/rsnapshot_backup.sh weekly; fi'"
	( set -o pipefail && stdbuf -oL -eL ssh -o BatchMode=yes -o StrictHostKeyChecking=no -p ${TARGET_PORT} ${TARGET} "bash -c 'exec > >(tee /opt/sysadmws/rsnapshot_backup/rsnapshot_backup.log); exec 2>&1; if [ $(date +%u) = 1 ]; then /opt/sysadmws/rsnapshot_backup/rsnapshot_backup.sh weekly; fi'" | ccze -A | sed -e 's/33mNOTICE/32mNOTICE/' ) || GRAND_EXIT=1
	# Daily
	stdbuf -oL -eL echo ---
	stdbuf -oL -eL            echo CMD: ssh -o BatchMode=yes -o StrictHostKeyChecking=no -p ${TARGET_PORT} ${TARGET} "bash -c 'exec > >(tee /opt/sysadmws/rsnapshot_backup/rsnapshot_backup.log); exec 2>&1; /opt/sysadmws/rsnapshot_backup/rsnapshot_backup.sh daily'"
	( set -o pipefail && stdbuf -oL -eL ssh -o BatchMode=yes -o StrictHostKeyChecking=no -p ${TARGET_PORT} ${TARGET} "bash -c 'exec > >(tee /opt/sysadmws/rsnapshot_backup/rsnapshot_backup.log); exec 2>&1; /opt/sysadmws/rsnapshot_backup/rsnapshot_backup.sh daily'" | ccze -A | sed -e 's/33mNOTICE/32mNOTICE/' ) || GRAND_EXIT=1
