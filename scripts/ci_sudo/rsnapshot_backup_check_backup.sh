...
if [ "${RSNAPSHOT_BACKUP_TYPE}" = "SSH" ]; then
	...
	( set -x ; set -o pipefail && stdbuf -oL -eL salt --force-color -t ${SALT_TIMEOUT} ${TARGET} cmd.run "bash -c 'exec > >(tee /opt/sysadmws/rsnapshot_backup/rsnapshot_backup.log); exec 2>&1; /opt/sysadmws/rsnapshot_backup/check_backup.sh 2'" | ccze -A ) || GRAND_EXIT=1
else
