...
if [ "_$1" = "_" -o "_$2" = "_" -o "_$3" = "_" ]; then
	echo ERROR: needed args missing: use rsnapshot_backup_check_backup.sh TIMEOUT TARGET TYPE
	exit 1
fi
...
if [ "${RSNAPSHOT_BACKUP_TYPE}" = "SSH" ]; then
	stdbuf -oL -eL echo ---
	stdbuf -oL -eL            echo CMD: ssh -o BatchMode=yes -o StrictHostKeyChecking=no -p ${TARGET_PORT} ${TARGET} "bash -c 'exec > >(tee /opt/sysadmws/rsnapshot_backup/rsnapshot_backup.log); exec 2>&1; /opt/sysadmws/rsnapshot_backup/check_backup.sh 2'"
	( set -o pipefail && stdbuf -oL -eL ssh -o BatchMode=yes -o StrictHostKeyChecking=no -p ${TARGET_PORT} ${TARGET} "bash -c 'exec > >(tee /opt/sysadmws/rsnapshot_backup/rsnapshot_backup.log); exec 2>&1; /opt/sysadmws/rsnapshot_backup/check_backup.sh 2'" | ccze -A | sed -e 's/33mNOTICE/32mNOTICE/' ) || GRAND_EXIT=1
