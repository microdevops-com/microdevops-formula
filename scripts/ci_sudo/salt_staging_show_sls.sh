...
# Update local repo
stdbuf -oL -eL echo "---"
stdbuf -oL -eL echo "NOTICE: CMD: git -C ${WORK_DIR}/srv pull || ( mkdir -p ${WORK_DIR}/srv && cd ${WORK_DIR}/srv && git init . && git remote add origin $2 )"
( stdbuf -oL -eL git -C ${WORK_DIR}/srv pull || ( stdbuf -oL -eL mkdir -p ${WORK_DIR}/srv && stdbuf -oL -eL cd ${WORK_DIR}/srv && stdbuf -oL -eL git init . && stdbuf -oL -eL git remote add origin $2 ) ) || GRAND_EXIT=1
