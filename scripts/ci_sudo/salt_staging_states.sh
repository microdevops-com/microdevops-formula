...
( set -x ; cd /srv && stdbuf -oL -eL git pull --ff-only && stdbuf -oL -eL git checkout -B $1 origin/$1) || GRAND_EXIT=1
( set -x ; cd /srv && stdbuf -oL -eL git submodule update --recursive -f --checkout ) || GRAND_EXIT=1
( set -x ; cd /srv && stdbuf -oL -eL ln -sf ../../.githooks/post-merge .git/hooks/post-merge ) || GRAND_EXIT=1
( set -x ; cd /srv && stdbuf -oL -eL .githooks/post-merge ) || GRAND_EXIT=1

# Run app.deploy
( set -x ; stdbuf -oL -eL salt --force-color -t 300 $2 state.apply cloud.pkg ) || GRAND_EXIT=1
# Run highstate
( set -x ; stdbuf -oL -eL salt --force-color -t 300 $2 state.highstate ) || GRAND_EXIT=1
# Run app.deploy
( set -x ; stdbuf -oL -eL salt --force-color -t 300 $2 state.apply app.deploy ) || GRAND_EXIT=1
