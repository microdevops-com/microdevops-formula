...
stdbuf -oL -eL echo ---
stdbuf -oL -eL echo CMD: salt --force-color -t 300 ${TARGET} state.apply rsnapshot_backup.update_config queue=True
stdbuf -oL -eL  bash -c "salt --force-color -t 300 ${TARGET} state.apply rsnapshot_backup.update_config queue=True" || GRAND_EXIT=1
