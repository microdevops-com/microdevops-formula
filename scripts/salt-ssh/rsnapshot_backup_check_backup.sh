...
if [ "${RSNAPSHOT_BACKUP_TYPE}" = "SSH" ]; then
	...
	( set -x ; set -o pipefail && stdbuf -oL -eL salt-ssh --wipe --force-color ${TARGET} cmd.run "bash -c 'exec > >(tee /opt/sysadmws/rsnapshot_backup/rsnapshot_backup.log); exec 2>&1; /opt/sysadmws/rsnapshot_backup/check_backup.sh 2'" | ccze -A | sed -e 's/33mNOTICE/32mNOTICE/' ) || GRAND_EXIT=1
else
